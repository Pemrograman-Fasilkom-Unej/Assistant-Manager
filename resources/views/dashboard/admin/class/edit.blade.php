    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="" method="post" action="{{ route('admin.class.update', $class) }}">
                        @csrf
                        @method('PUT')
                        <div class="form-group fill">
                            <label class="floating-label" for="name">Nama Kelas</label>
                            <input type="text" class="form-control" id="name" aria-describedby="Nama Kelas" placeholder="Masukan Nama Kelas" name="title" required value="{{ $class->title }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="js-example-basic-single form-control" id="semester-select" name="semester" required>
                                <option {{ $class->semester == 1 ? 'selected' : '' }} value="1">Ganjil</option>
                                <option {{ $class->semester == 2 ? 'selected' : '' }} value="2">Genap</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select multiple="multiple" class="js-example-basic-multiple form-control" id="assistants-option" name="assistants[]" required>
                                @foreach($assistants as $assistant)
                                    <option value="{{ $assistant->id }}" {{ in_array($assistant->id, $class->assistants->pluck('assistant_id')->toArray()) ? 'selected' : '' }}>{{ $assistant->name }}</option>
                                @endforeach
                            </select>
                        </div>
