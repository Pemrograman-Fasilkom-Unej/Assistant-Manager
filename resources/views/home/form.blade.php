...
<html lang="en">
<head>
    <title>Assistant Manager - {{ $task->title }} </title>
    <meta charset="utf-8">
...
<div class="auth-wrapper maintance">
    <div class="container mt-5 mb-5">
        <div class="row justify-content-center">
            ...
            @if(\Carbon\Carbon::now()->isBefore($task->due_time))
                                ...
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane" id="progress-t-tab3">
                                        <form class="text-center">
                                            ...
                                            <p>Terima kasih telah mengumpulkan tugas <b>{{ $task->title }}</b> pada
                                                kelas <b>{{ $task->classes->title }}</b></p>
                                        </form>
                                    </div>
...
<script>
    ...
    $('#form-wizard').bootstrapWizard({
        ...
        onNext: function (tab, navigation, index) {
            if (index === 1) {
                var status_student = false;
                if (!$('#nim-input').val()) {
                    $('#nim-input').focus();
                    $('.form-1').addClass('was-validated');
                    notify("Silahkan isi NIM", "danger");
                    return false;
                } else {
                    $.post({
